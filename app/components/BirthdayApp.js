...
import { ChevronLeft, ChevronRight, Heart, Sparkles, Gift as GiftIcon, Stars, Music, VolumeX, Infinity, Flame, HeartHandshake, Sparkle, Clock, BookHeart, Cake as CakeIcon, Ticket, Flower2, Crown, Map, Star } from 'lucide-react';
...
const RevealCard = ({ gift, isOpened }) => (
  isOpened ? (
  <div className="fixed left-1/2 sm:left-3/4 lg:left-4/5 top-2/3 z-50 w-[min(18rem,80vw)] -translate-x-1/2 -translate-y-1/2 animate-gift-reveal pointer-events-auto max-h-[80vh] overflow-y-auto">
    <div className={`relative rounded-2xl sm:rounded-3xl border-2 ${gift.revealBorder} bg-gradient-to-br ${gift.revealBg} px-2 sm:px-6 pb-3 sm:pb-7 pt-12 sm:pt-14 text-center shadow-[0_25px_60px_rgba(225,29,72,0.24)] overflow-visible`}>
      <div className="absolute -top-5 sm:-top-6 left-1/2 flex h-12 sm:h-14 w-12 sm:w-14 -translate-x-1/2 items-center justify-center rounded-full border-2 border-white bg-gradient-to-br from-white to-pink-50 shadow-lg z-10">
        <Heart className={`h-6 sm:h-7 w-6 sm:w-7 ${gift.accent}`} fill="currentColor" aria-hidden="true" />
      </div>
      
      <div className="mb-1 sm:mb-2 flex justify-center gap-1">
        <Sparkles className={`h-3 sm:h-4 w-3 sm:w-4 ${gift.accent}`} aria-hidden="true" />
        <Sparkles className={`h-3 sm:h-4 w-3 sm:w-4 ${gift.accent}`} aria-hidden="true" />
        <Sparkles className={`h-3 sm:h-4 w-3 sm:w-4 ${gift.accent}`} aria-hidden="true" />
      </div>
...
const App = ({ startOpened = false, showIntroMessageOnOpen = true, onSealOpen } = {}) => {
 ...
 // Night Celebration State
  ...
  const specialSurpriseVideoDirectUrl = 'https://drive.google.com/file/d/1LtYkifoFurEamQaAmWYg2N2os0LKVGVY/';
      ...
      {showSpecialSurpriseModal && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/70 p-3 backdrop-blur-md sm:items-center sm:p-4" style={{ animation: 'fadeIn 0.3s ease-out forwards' }}>
          <div className="relative flex max-h-[calc(100dvh-1.5rem)] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] border border-white/18 bg-gradient-to-br from-slate-950 via-violet-950 to-rose-950 text-white shadow-[0_32px_120px_rgba(15,23,42,0.58)] sm:max-h-[90vh]" style={{ animation: 'scaleIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards' }}>
            ...
            {/* Content */}
            <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center gap-5 overflow-y-auto px-4 pb-5 sm:px-7 sm:pb-7">
              {/* Video Player - Optimized for Fast Streaming */}
              ...
              <div className="w-full max-w-2xl shrink-0 space-y-4 text-center">
                ...
                {/* Direct Play Button */}
                <a
                  href={specialSurpriseVideoDirectUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 rounded-xl px-6 py-4 bg-gradient-to-r from-cyan-500 via-blue-500 to-blue-600 text-white font-nunito text-sm font-bold uppercase tracking-[0.14em] transition-all duration-300 hover:shadow-lg hover:scale-105 shadow-md w-full"
                >
                ...
                <div className="rounded-2xl border border-cyan-200/22 bg-cyan-100/10 p-5 shadow-sm backdrop-blur">
                  <p className="text-sm text-cyan-100 font-nunito font-bold">
                    💡 For best quality with large files, use the Play Video Now button
                  </p>
